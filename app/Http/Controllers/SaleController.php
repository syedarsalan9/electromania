...
class SaleController extends Controller
{
            ...
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="'.route('sale.invoice', ['id' => $sale->id]).'" class="btn btn-link"><i class="fa fa-copy"></i> Generate Invoice(without txn)</a></li>
                                <li><a href="http://electromania.local/sales/gen_pdf/'.$sale->id.'" class="btn btn-link"><i class="fa fa-copy"></i>Generate Pdf (without txn)</a></li>
                                <li><a href="http://electromania.local/sales/gen_txn_pdf/'.$sale->id.'" class="btn btn-link"><i class="fa fa-copy"></i>Generate Pdf(with TXN)</a></li>
                                <li><a href="http://electromania.local/sales/txn_invoice/'.$sale->id.'" class="btn btn-link"><i class="fa fa-copy"></i>Generate Inovice(with Txn)</a></li>
                                <li>
    ...
    public function store(Request $request)
    {
        ...
        $request['grand_total'] = $request['grand_total'];
        $data = $request->all();
        //return dd($data);
    ...
    {   
        ...
        if(\App::getLocale() == 'ar' || \App::getLocale() == 'hi' || \App::getLocale() == 'vi')
            ...
            $numberTransformer = $numberToWords->getNumberTransformer(\App::getLocale());
        $numberInWords = $numberTransformer->toWords($lims_sale_data->grand_total);
        $pdf = PDF::loadView('sale.pdf_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
            // download pdf
        return $pdf->download('invoice.pdf');
    }

    public function genTxnPdf($id)
    {   
        ...
        if(\App::getLocale() == 'ar' || \App::getLocale() == 'hi' || \App::getLocale() == 'vi')
            ...
            $numberTransformer = $numberToWords->getNumberTransformer(\App::getLocale());
        $numberInWords = $numberTransformer->toWords($lims_sale_data->grand_total);
        $pdf = PDF::loadView('sale.pdf_txn_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
            // download pdf
        return $pdf->download('invoice.pdf');
    }

    
    public function genInvoiceTxn($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        return view('sale.txn_invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
